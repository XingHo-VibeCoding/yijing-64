...
### 5.3 代码与数据的差距（G1–G6 已修复 / G7 待做）

**2026-09-21 记录时的状态**：数据已补齐，但界面还停留在旧状态。原登记 7 项差距，**G1–G6 当晚已修完**：

| # | 差距 | 状态 | 实现方式 |
| --- | --- | --- | --- |
| G1 | 详情页仍显示「待补内容」块，而数据已入库 | ✅ **已修** | 换成真实的卦辞 / 象辞 / 六爻爻辞；「待补」只保留**白话**一项，并写明为什么不放第三方白话 |
...
| G7 | F8 每日第一卦存档、收藏均未实现 | ⏳ **待做** | 见 §15 第 0b 步；属新功能，不是「界面落后于数据」 |
...
| 0b | **F8：每日第一卦存档 + 收藏**（纯前端 localStorage 起步，不碰云） | 我 | — |
