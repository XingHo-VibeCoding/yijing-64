...
> **项目名称**：易经六十四卦学习
> 文档版本：**v2.5** ｜ 日期：2026-09-22 ｜ 阶段：Day 6（**v2.5：随 PRD v1.4 砍掉白话层**）
>
> **v2.5 是 v2.4 的追加**：PRD v1.4 决定砍掉「白话译文」整层（PRD §4.2），本文件同步 ——
> §5.1 / §5.2 的字段与来源标注由三类收为**两类**，§5.3 新增 **G8（白话层清理，待做）**，
> §13.3 原「补上白话」一行改为**取消**。**代码与数据尚未改**，见 G8。
>
...
| | 来源标注 + 白话「暂无」说明块 —— ⚠️ **v2.5**：PRD v1.4 已砍掉白话整层，该说明块与 `plain` 标注类型**随之移除**（数据 / 代码待改，见 §5.3 **G8**） | F5 |
| **调试开关** | `?castSpeed=0.5` 半速播放；`?castFreeze=10100` 定格在 10.1 秒——**仅供开发核对，不是产品功能** | — |

### 5.2 数据文件与字段

**① `data/64卦.json`** —— 顶层 `{ meta, items }`，`items` 64 条，每条 13 个字段（**v2.5：其中白话 2 个字段随白话层移除，清理后为 11 个**，见 §5.3 G8）：
...
| `pinyin` | str | 卦名拼音 | **待补** |
| ~~`judgmentPlain` / `imagePlain`~~ | — | ~~卦辞 / 象辞白话~~ | **v2.5 移除**（白话层砍掉，PRD v1.4 §4.2；清理见 G8） |
...
> **数据字段、界面说明块、`plain` 标注类型一并移除**（清理清单见 §5.3 **G8**）。

### 5.3 代码与数据的差距（G1–G7 已修 / G8 待做：白话层清理）
...
| **G8** | **白话层的清理**（**v2.5 新增**）：PRD v1.4 砍掉白话后，代码与数据里还留着这一层 | ⏳ **待做**（**不属今日范围 —— Day 6 不写代码**） | ① `src/App.jsx` / `src/CastResult.jsx` 里的「白话译文暂无」说明块 → 移除；② `src/SourceTag.jsx` 的 `plain` 类型 → 删除，来源标注由**三类收为两类**；③ `data/64卦.json` 的 `judgmentPlain` / `imagePlain` → 与 PRD §7.1 同步删除（**改数据前先按 §13.3 列影响面**） |
...
| ~~补上白话（`judgmentPlain` 等）~~ | — | **v2.5 取消**：白话整层砍掉（PRD v1.4 §4.2）。反向的**清理**工作见 §5.3 **G8** |
