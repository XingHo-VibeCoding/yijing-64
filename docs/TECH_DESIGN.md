...
> **项目名称**：易经六十四卦学习
> 文档版本：**v2.16** ｜ 日期：2026-09-26 ｜ 阶段：Day 8（**v2.16：竹简风做旧修订 —— 离散竹片 + 深色做旧木牌 + 全部纸质做旧**）
>
...
│  ├─ InkTransition.jsx        # 总表→详情的墨侵染转场（v2.15：石头入水涟漪/侵染/墨散/竹简展开/墨起浮现，?transFreeze 逐帧核对）
...
| **传统竹简风改版**（Day 8 追加） | **v2.15 新增（用户点题：不要简约风）**：① **总表** = 竖向竹条背景上的**宣纸**页面，每卦一行做成**木牌**（令牌）造型（上圆下方 `border-radius:11px 11px 5px 5px` + `::before` 挂孔），「起一卦 / 我的记录 / 测一测」做成**木匾**（起一卦为深色主匾）；② **详情页** = **竹简记录**风（`page-slips`：竹简底 + 半透宣纸压字，原文仍衬线字体栈）；③ **转场**改为用户描述的时序——**石头投入水激起的波动涟漪 → 淡墨侵染 → 墨散 → 竹简展开占满整屏（`scaleX .04→1`，`repeating-linear-gradient` 模拟竹条）→ 内容以「墨起」浮现（radial-gradient 暗层渐褪）**，取代 v2.14 的「卦象中央浮现 → 归位」。传统材质色板集中在 `index.css` 末尾（`--paper` 宣纸 / `--wood` 木牌 / `--slip` 竹简 / `--seal` 印泥红），靠层叠覆盖前面的简约样式。**实测（截图见 `.workbuddy/checkin/day8e/`）**：总表宣纸 + 木牌 ✓、详情竹简 ✓、转场定格六帧（浓墨完全遮挡 → 竹简浮现 → 墨起渐清晰）✓、完整跑完无残留 ✓ | F1 / F2 |
...
| **v2.15** | 2026-09-26 | **整体改版：中国传统竹简风 + 转场时序按用户描述重排（Day 8 追加，用户点题「不要简约风」）。** ① **总表**：body 改竖向竹条背景，内容区为**宣纸**，每卦一行做成**木牌**（令牌）造型（上圆下方 + 挂孔），三个入口按钮做成木匾（起一卦深色主匾）；② **详情页**：**竹简记录**风（竹简底 + 半透宣纸压字）；③ **转场**：涟漪改为**石头投入水激起的波动感**，墨散后先**竹简展开占满整屏**（`scaleX .04→1`），内容再以「**墨起**」形式浮现 —— 取代 v2.14 的「卦象中央浮现 → 归位」两步。实现集中在 `InkTransition.jsx`（时间轴重排 + `.ink-trans-scroll` / `.ink-trans-wash` 两层）与 `index.css` 末尾的传统材质变量（`--paper`/`--wood`/`--slip`/`--seal`，靠层叠覆盖简约样式）。**实测（截图 `.workbuddy/checkin/day8e/`）**：总表宣纸木牌、详情竹简、转场定格六帧（浓墨完全遮挡 → 竹简浮现 → 墨起渐清晰）、完整跑完无残留，全部通过 |
| **v2.16** | 2026-09-26 | **竹简风做旧修订（Day 8 追加，用户给三张实物样例后点题）。** 用户反馈：颜色要更深（深棕）、要「竹简样式」而非细密竹纹理、木牌要深色做旧木纹、所有纸质纹理都要做旧。逐项落地：① **竹简 = 一片片离散竹片**（每片 46px + 近黑缝 6px，片内有弧面明暗：中间受光、两缘入影）+ 相邻竹片深浅交替 + 纵向竹纤维细丝 + 横向年久色斑，详情页与转场展开层同用这一套（`.page-slips` / `.ink-trans-scroll`）；② 详情页顶部加**编绳**（`::before` 一根穿过竹片的绳）；③ **木牌全部改深色做旧木**（`#4c3016` 底 + 斜向木纹 + 内阴影），文字反转为浅色（`#f1e4c8` 系）保证可读，`item-*` 四个子元素与「我的记录 / 返回」按钮一并反转；④ **纸质全部做旧**：总表/记录页宣纸与竹简上的宣纸条加茶渍斑（多层 radial-gradient）+ 边缘压暗，起卦动画背景与结果页宣纸底衬同步做旧（`casting.css`）；⑤ 全局变量整体加深（`--slip-1 #9a6a33`、`--slip-2 #1c0e04`、`--wood-1 #4c3016`、`--paper-1 #e9d9b4`、body 深棕木桌面 `#2f1d0d` + 四角压暗）。**实测（截图 `.workbuddy/checkin/day8f/`，用系统 Edge 无头 `--screenshot` 拍摄 —— agent-browser 限流期间的替代手段）**：总表深棕做旧木牌 + 做旧宣纸 ✓、详情离散竹片 + 深缝 + 编绳 ✓、记录页 ✓，文字全部清晰可读 |
...
| **v2.17** | 2026-09-26 | **Day 9 设计审查与修复（改动仅 `src/index.css`）。** 用 WCAG 对比度实测脚本逐元素算比值，查出 **Day 8 木牌 / 竹简改版是「文件尾部覆盖式」做的、有 5 处样式没跟着迁移**：① 三处关键标签（卦辞/象辞、爻题、上卦/下卦）仍是旧版冷灰 `#8c959f`，压在宣纸底上只有 **2.19–2.28** 的对比度（11.5–12px 小字要求 ≥4.5）；② `.volume-range` 4.06、`.yao-label` 4.31 偏弱；③ 首页三个入口（起一卦 / 我的记录 / 测一测）同宽同形同为深色木匾，且与卦列表同材质同宽度 —— **主操作被淹掉**；④ 手机端详情页顶栏换行后「收藏」孤立左对齐；⑤ 禁用翻页按钮 `opacity:.38` 连边框一起淡进纸里、几乎不可见。修复方式：① ② 迁到既有暖色 token（实测 5.13–5.79）；③ 次级入口改用 `.page-slips .block` 同款做旧宣纸条、主操作保持深色木匾；④ 手机宽度用 `order` 重排（返回←→收藏同行两端、翻页独占一行靠右）；⑤ 禁用态改「纸底 + 虚线边框 + 只淡化文字」。**验证**：修复后全部元素对比度 ≥4.92、块间距 26/26/26 一致、375px 视口无横向溢出、功能回归 7 项全部通过；修复前后各抓同视口同位置截图，并注入还原样式重跑审计（数字精确回到 2.28/2.19/4.31）以证明对照成立 |
| **v2.18** | 2026-09-26 | **转场按用户反馈修订（InkTransition.jsx + index.css）。** ① **动画结束前不露出详情页**：查出卷轴的透明度原写成 `open × opacity` —— 展开时整个卷轴是半透明的，底下的详情页一路透出来；改为**全程不透明、展开完保持占满**，且页面透明度整段压在 0.16、模糊保底 6px，只在最后一层墨起盖到峰值之后才随墨一起亮起来；② **卷轴展开放慢**：1.2 秒 → **2.2 秒**；③ **卷轴边缘淡黑墨流动**：新增画在卷轴之上的独立画布（`.ink-trans-edge`），墨浓在卷轴内侧、随展开往下淌（带底 + 5 条下移墨痕 + 6 道墨滴）；④ **涟漪更慢更有诗意**：1.2 秒 → 2 秒，波圈带角向扰动（不再正圆）、加二次溅落的淡波与抛物线水珠；⑤ 总表卦牌/起卦入口从 92° 细密交叉纹（**布纹**）改为顺纹木纹配方（`--wood-grain`：横向生长轮 + 宽色带 + 深纹线，卦牌每 4 块 2 块带木节）。全转场 6 秒 → 7.9 秒。实测：`?transFreeze` 10 帧 + 页面透明度全程 ≤0.20（7050 墨起峰值时满屏卷轴）、真实跑完落在详情页且无残留模糊 |
| **v2.19** | 2026-09-27 | **转场再修订：竹简出现时机与立体感（InkTransition.jsx + index.css）。** ① **「出现太早」的实锤**：主循环每帧都把卷轴 opacity 写成 1（只有结尾才淡出），那条 scaleX(0.04) 的细缝从第 0 帧就立在屏幕中央 —— 改为随**墨散**（3600–4600）淡入（3850 起 550ms），4500 墨将散尽才开始展开；② **立体感**：展开从 scaleX（纹理横向压扁再拉宽，像拉贴图）改为 **clip-path 从中间向两侧揭开**（纹理不变形，才是展卷）；新增**两根卷轴轴杆**（圆柱明暗渐变 + 上下轴头 + 投影，贴着展开边随展开外推）；简面加**弧面受光**（中间亮、两侧渐暗）与上下卷曲暗带。时间轴顺延：全转场 7.9 秒 → 8.0 秒。实测：transFreeze 定格 1500（卷轴 op=0 不可见）/ 4200（墨散中淡入 0.64、仍是卷着的细卷）/ 4800（展开 35%、轴杆外推）/ 7100（墨起峰值满屏盖住详情页）；真实跑完落在详情页、无残留模糊 |
