...
> **项目名称**：易经六十四卦学习
> 文档版本：v2.0 ｜ 日期：2026-09-21 ｜ 阶段：Day 5 技术方案（按清单补齐）
...
| 触发条件 | 动作 |
| --- | --- |
| V3 证实 PG 的 CPU **按常驻计费**（§14.2） | **立刻回退方案 A**：前端全本地存储，F6/F8 的云端部分暂缓。**产品功能一点不受影响**，只是回到 PRD v1.0 的存储方式，并在 PRD 记一笔 |
...
```
yijing-64/
├─ index.html                  # Vite 入口（12 行）：中文 lang、viewport、挂载 #root
├─ package.json                # 依赖：react/react-dom ^19.3；devDep：vite ^8.3 + @vitejs/plugin-react ^6.1
├─ vite.config.js              # 端口 5173、strictPort；**无代理、无后端**
├─ README.md                   # 项目说明、产品边界、进度表
├─ .gitignore                  # 排除 .workbuddy/ · Claw/ · .env · 密钥 · node_modules · dist
│
├─ src/                        # 前端源码（共 761 行）
│  ├─ main.jsx                 # 挂载入口（10 行）
│  ├─ App.jsx                  # 总表页 + 详情页 + 起卦逻辑 castOnce()（142 行）
│  ├─ CastingAnimation.jsx     # F7 起卦动画：太极 + 三圈爻 + 虚圆 + 水墨（462 行）
│  ├─ HexagramFigure.jsx       # 六爻卦象图组件，从下往上渲染（40 行）
│  ├─ index.css                # 主样式：总表、详情、起卦按钮（140 行）
│  └─ casting.css              # 起卦动画样式：宣纸底、虚圆、动画按钮（107 行）
│
├─ data/                       # 数据（前端直接 import，不走网络）
│  ├─ 64卦.json                # 64 卦：结构 + 卦辞 + 象辞 + 吉凶平（23KB）
│  ├─ 64卦-爻辞.json           # 384 条爻辞 + 小象（56KB）
│  └─ 64卦总表.csv             # 原始索引：序号 / 卦名 / 卦符 / 篇章（1.2KB）
│
├─ docs/                       # 文档（与代码同等重要）
│  ├─ research.md              # Day 3 需求研究：目标用户、竞品、25 天最小版本
│  ├─ PRD.md                   # Day 4 产品需求 v1.3：功能详述、验收标准
│  ├─ TECH_DESIGN.md           # ★ 本文件
│  └─ 01-学习笔记.md            # 逐卦笔记模板
│
...
| | 按上经（1–30）/ 下经（31–64）分组的 64 项列表，每项含**序号 + 卦符 + 卦名** | F1 |
| | 页脚「共 64 卦 · 每卦含六爻爻线与上下卦 · 数据来源：…」 | F5 |
| **起卦动画**（F7） | 第一段：太极双鱼**逆时针** + **三圈交错阴阳爻**（不同圈不同方向、速度不同）+ 两条**虚圆**，9 秒后同步停住并**归回标准位置** | F7 |
| | 墨从四周翻涌遮住图案 → **保持 1 秒** → 散去并浮现卦象 | F7 |
| | 第二段（点击触发）：「看这一爻」/「再看一遍」→ 墨再涌 → 散去后**突出确定的爻、其余淡化至 10%** | F7 |
| | 出口按钮「查看解读 →」 | F7 |
| | 背景黑墨**全程翻涌旋转**（含动画结束后），中心浓周边淡，不遮挡图案 | F7 |
| **详情页** | 返回按钮、卦头（大卦符 + 卦名 + 第 N 卦 + 篇章） | F2 |
| | **六爻卦象图**（`HexagramFigure`）+ 上卦 / 下卦及其自然象 | F2 |
| | **来源标注组件**「《周易》原文 / 现代白话解读」 | F5 |
| | 「待补内容」提示块 | —（见 §5.3 缺口） |
| **调试开关** | `?castSpeed=0.5` 半速播放；`?castFreeze=10100` 定格在 10.1 秒——**仅供开发核对，不是产品功能** | — |

### 5.2 数据文件与字段

**① `data/64卦.json`** —— 顶层 `{ meta, items }`，`items` 64 条，每条 13 个字段：
...
| `pinyin` | str | 卦名拼音 | **待补** |
| `judgmentPlain` / `imagePlain` | str | 卦辞 / 象辞白话 | **待补（留空）** |
...
> **白话一律留空，是有意为之**：第三方白话有版权与准确性风险，本项目**不搬运、不自撰**，宁可显示「待补」。

### 5.3 代码与数据的差距（已知缺口，本轮不动代码）

数据已经补齐，但**界面还停留在旧状态**。按今日清单「今日不做：写代码」，只登记不改：

| # | 缺口 | 影响 |
| --- | --- | --- |
| G1 | 详情页仍显示「待补内容」块（卦辞 / 象辞 / 爻辞），**而这些数据已入库** | 数据白准备了，用户看不到 |
| G2 | 详情页未展示 `judgment` / `image` / `fortune` | F2 未达标 |
| G3 | 起卦结果页缺**「进入卦象爻辞总表」并定位高亮本卦**这个交互键 | F7 的「把好奇导向学习」缺一半 |
| G4 | 结果页缺**三层解读**（卦象说什么 → 卦辞爻辞怎么讲 → 想跟你说的话） | F7 核心未实现 |
| G5 | 结果页缺**边界声明**（首页有，结果页没有） | §1.3 要求「两处声明」 |
| G6 | `fortune` 已入库但**界面完全未使用** | 吉卦戒骄 / 凶卦劝慰这条线未生效 |
| G7 | F8 每日第一卦存档、收藏，均未实现 | 留存机制未建立 |
...
| # | 待核实项 | 冲突点 | 影响 |
| --- | --- | --- | --- |
| **V1** | **PG 支持哪些地域** | 腾讯云计费文档写「上海地域**不支持** PG，仅新加坡支持」；同站产品页写「当前支持上海、新加坡两个地域」——**两处官方口径互相矛盾**（PG 是 2026-08 新上线，文档可能未同步） | 若只能选新加坡：中国大陆用户访问延迟升高。**只影响延迟，不阻断方案** |
| **V2** | **免费档能否用静态托管** | 早期 CloudBase Framework 文档提过「Website 插件需要环境为**按量计费**模式」；当前定价页把 CDN / 静态托管列为免费档包含能力 | 若不能：**部署方案要改**（前端托管他处，数据库仍在 CloudBase） |
| **V3** | **PG 计费与空闲行为** | 计费项为 `PostgreSQL CPU 使用量 342 点/核·小时`；官方博客称「应用空闲时会自动挂起计算资源并保留数据」——但**空闲判定标准与挂起延迟未说明** | 若 CPU 持续计费，`720 小时 × 342 点 = 246,240 点` 会**瞬间击穿 3,000 点免费额度**。**这是本方案最大的成本风险**，见 §14 |
| **V4** | **RLS 当前用户函数名 / REST 路径前缀 / 前端 SDK 包名** | Supabase 用 `auth.uid()`；CloudBase PG 宣称兼容 Supabase 语法，但**函数名未在查到的文档中确认** | 决定 RLS 策略怎么写、前端怎么调。**未确认前不要照抄 §7.3 与 §9 的示意代码** |
...
```mermaid
flowchart TB
    subgraph FE["浏览器 · 前端（React + Vite 静态站点）"]
        UI["页面<br/>总表 / 详情 / 起卦动画"]
        LOCAL["本地数据<br/>64卦.json · 64卦-爻辞.json<br/>打包进前端 · 不走网络"]
        CALC["起卦计算<br/>本地随机 · 不联网"]
        LS["localStorage<br/>暂存待上传 + 本地副本"]
        UI --> LOCAL
        UI --> CALC
        UI -- "③ 云端不可用时暂存本地" --> LS
        LS -- "④ 下次打开读回，取较新的一份" --> UI
    ...
    subgraph CB["CloudBase 免费环境"]
        AUTH["匿名登录"]
        REST["PostgREST<br/>建表即得 REST 接口"]
        RLS["RLS 行级权限<br/>只能碰 uid 等于自己的行"]
        DB["PostgreSQL<br/>divination_records / favorites / study_progress"]
        REST --> RLS
        RLS --> DB
    end
    ...
    UI -- "① 首次访问申请匿名身份" --> AUTH
    AUTH -- "② 返回用户标识 uid" --> UI
    UI -- "③ 写入今日第一卦 / 收藏 / 进度" --> REST
    DB -- "④ 只取回该 uid 自己的行" --> REST
    REST -- "⑤ 返回 JSON" --> UI
```
...
```mermaid
flowchart LR
    A["起卦完成"] --> B{"今天已经有<br/>第一条记录了吗"}
    B -- "有" --> C["只在本地展示<br/>不入档 · 不提示<br/>（PRD F8 每日只记第一卦）"]
    B -- "没有" --> D["POST /divination_records"]
    D --> E{"写入成功吗"}
    E -- "成功" --> F["本地与云端一致"]
    E -- "失败 / 超时" --> G["写入 localStorage 暂存<br/>页面不弹错误框<br/>后台静默重试"]
    G -. "网络恢复" .-> D
```
...
| E7 | 生僻字字体缺失（如 `𬙊` `𫗧` `𦈡` 三个扩展区字） | 显示成豆腐块 | 需**指定支持扩展区的字体**或对这类字做兜底 | 界面自然体现 |
...
| --- | --- | --- |
| **V3 证实按常驻计费** → 回退纯本地存储 | PRD §7.3（存储方式）、`TECH_DESIGN` §2.4 / §14、`App.jsx`（读写逻辑）、本表 | 无需现在做，**红线已写明** |
| **V2 证实免费档不能托管** → 前端改托管他处 | `TECH_DESIGN` §13.1、README 技术栈行、PRD §3.2 | 无需现在做 |
| **V1 证实只有新加坡** | `TECH_DESIGN` §6.2 / §12.2（`VITE_TCB_REGION`）、README | 环境创建时确认 |
...
| **补上白话（`judgmentPlain` 等）** | `data/64卦.json` + `App.jsx` 详情页 + PRD §7.2 内容红线（白话须标注来源） | ⚠️ 涉版权，**必须先解决来源问题** |
...
| 环境数 | 每账号 1 个免费环境 |
| **按量计费** | **官方明确：免费档不可用资源包与按量计费** ← 天然满足硬约束 1 |
| 续期 | 免费档需续期，**有到期时间**（具体以控制台为准） |

### 14.2 ★ 最大成本风险：PG 的 CPU 计费
...
| 步 | 动作 | 谁做 | 是否需授权 |
| --- | --- | --- | --- |
| 0 | **修复 §5.3 的 G1–G6**（界面落后于数据的部分）：详情页展示卦辞/象辞、起卦结果页三层解读 + 两个交互键 | 我 | — |
...
| 5 | 建表 + 配 RLS（执行 §7 的 SQL） | 我 | ⛔ 涉及建资源，**执行前给你看 SQL** |
...
> **顺序上的一个说明**：第 0 步刻意排在最前面。**理由是先让产品自己站得住，再考虑云端**——万一 V3 的结果是「不划算」，第 0 步的成果完全保留，不会白做。
...
| # | 待确认 | 怎么确认 |
| --- | --- | --- |
| V1 | PG 支持哪些地域 | 在购买/创建环境页面**亲眼确认**上海在不在列表里 |
| V2 | 免费档能否用静态网站托管 | 创建环境后看控制台是否出现「静态网站托管」入口 |
| V3 | **PG 计费口径与空闲挂起**（最关键） | 建库后**观察 24 小时资源点消耗**再继续开发 |
| V4 | RLS 当前用户函数名 / REST 路径前缀 / 前端 SDK 包名 | 查 CloudBase PG 官方接口文档 |
...
| V3 | **PG 计费口径与空闲挂起** | 待核实 | **最大成本风险**；见 §14.3 红线 3、4 与 §2.4 回退条件 |
...
| R4 | **界面落后于数据**（§5.3 G1–G6） | 已知 | 第 0 步修复；**不修则 F2 / F7 验收不通过** |
| R5 | 生僻字字体缺失（3 个扩展区字） | 已知 | §11.2 E7；做详情页时指定字体或兜底 |
...
| v2.0 | 2026-09-21 | 按 Day 5 任务清单补齐六块：**§2 方案 A / 方案 B 四维比较**（学习成本 / 线上持久化 / 费用 / 排错难度，推荐 B 并写明「B 是 A 的超集」及回退条件）、**§4 项目结构**、**§5 已做网页内容与字段**（含 7 项「界面落后于数据」的已知缺口）、**§10 数据流图**（Mermaid，另加起卦写入的错误分支流程图）、**§12 环境变量**（结论：前端不需要密钥）、**§13.2 迁移注意事项**。另新增 §13.3「方案变化时受影响文件」表（余力加练）、§16 待确认问题清单（含 Q1–Q5 三项限制的追问）、§15 第 0 步「先修界面，再碰云端」 |
