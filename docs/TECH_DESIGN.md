...
**① `data/64卦.json`** —— 顶层 `{ meta, items }`，`items` 64 条，每条 13 个字段（**v2.5 实测更正：白话字段从来没有进过数据** —— 只在 `meta.pending` 里登记过名字；`pinyin` 也未补）：
...
| **G8** | **白话层的清理**（**v2.5 新增**）：PRD v1.4 砍掉白话后，代码与数据里还留着这一层 | ⏳ **待做**（**不属今日范围 —— Day 6 不写代码**） | ① `src/App.jsx` / `src/CastResult.jsx` 里的「白话译文暂无」说明块 → 移除；② `src/SourceTag.jsx` 的 `plain` 类型 → 删除，来源标注由**三类收为两类**；③ `data/64卦.json` 的 `meta.pending` 里仍登记着 `judgmentPlain` / `imagePlain` → 应从这份「待补清单」移除（**注意：这两个字段从未真正写进数据，所以没有数据要删** —— 2026-09-22 实测更正） |
